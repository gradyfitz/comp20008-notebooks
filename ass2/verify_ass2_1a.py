...
solution_output_name = "amazon_google_truth_small.csv"

first_data_set_name = "amazon_small.csv"
second_data_set_name = "google_small.csv"
...
def get_rows(df: pd.DataFrame, col_mapping: Dict[str, str], column_order: List[str]) -> List[List[str]]:
    ...
    for index, row in df.iterrows():
        ...
        for col in column_order:
            entry.append(row[col_mapping[col]].strip())
        rows.append(entry)
...
try:
    ...
    assert clean_string('idAmazon') in [clean_string(col) for col in student_df.columns], "column idAmazon missing"
    assert clean_string('idGoogleBase') in [clean_string(col) for col in student_df.columns], "column idGoogleBase missing"
    assert len(student_df.columns) == 2, "Unexpected columns {}".format([col for col in student_df.columns if clean_string(col) not in ['idAmazon', 'idGoogleBase']])
    ...
    lf1_df = pd.read_csv(first_data_set_name)
    lf2_df = pd.read_csv(second_data_set_name)
