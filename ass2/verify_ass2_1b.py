...
google_block_output_name = "google_blocks.csv"
amazon_block_output_name = "amazon_blocks.csv"
solution_output_name = "amazon_google_truth.csv"

google_data_set_name = "google.csv"
amazon_data_set_name = "amazon.csv"
...
def calculate_statistics(student_google_df: pd.DataFrame, student_amazon_df: pd.DataFrame, solution_df: pd.DataFrame, sol_google_df: pd.DataFrame, sol_amazon_df: pd.DataFrame) -> Tuple[int, int, int, int, int, Dict[str, Union[int, str]]]:
    ...
    for index, row in solution_df.iterrows():
        amazon_row = row['idAmazon']
        google_row = row['idGoogleBase']
...
try:
    ...
    sol_google_df = pd.read_csv(google_data_set_name)
    sol_amazon_df = pd.read_csv(amazon_data_set_name)
    ...
    print("\t\tGoogle ids in block: {}".format(largest_block_pair['google']))
    print("\t\tAmazon ids in block: {}".format(largest_block_pair['amazon']))
